...
struct Task(alias queueable, dependencies = DependsOn!())
    if(is(dependencies : DependsOn!X, X ...))
{
    alias work = queueable;

    alias dependencyTypes = dependencies;

    static if(isNativeFunction!queueable)
    {
        alias ArgTypes = ParameterTypeTuple!(queueable);
    }
    else static if(isCLKernel!queueable)
    {
        alias ArgsTypes = queueable.ArgTypes;
    }
    ...
    cl_command_queue[] defaultQueues;

    size_t[2][work.nParallelDims][] globalRange;
    size_t[work.nParallelDims][] localSize;
    ...
    auto instance(size_t queueIndex)
    {
        return TaskInstance(defaultQueues[queueIndex],
                globalRange[queueIndex],
                localSize[queueIndex],
                defaultArgs);
    }
}
...
{
    alias types = ExtractTypes!TaskTs;
}

struct TaskInstance(TaskT)
{
    //dependencies could be arrays of dependencies, for
    //variable length things
    cl_event[TaskT.dependencies.length] blockers;
    ...
    {
        cl_event ev;
        static if(isNativeFunction!(TaskT.work))
        {
            static assert(false, "not implemented");
        }
        else static if(isCLKernel!(TaskT.work))
        {
            clSetKernelArgs(TaskT.queueable, args);
            clEnqueueCLKernel(queue, TaskT.work,
...
struct Kernel(ArgTypes_ ...)
{
    cl_kernel id;
    alias id this;
 
    alias ArgTypes = ArgsTypes_;
...
auto enqueue(Task)(cl_command_queue queue, Task task, RangeInfo ri)
    if(isOpenCLTask!Task)
{
    cl_event event;
    size_t[
    clEnqueueCLKernel(queue, task.work,  task.dependsOn, &event);
    return event;
}

//looks up RangeInfo in the task
auto enqueue(Task)(cl_command_queue queue, Task task)
    if(isOpenCLTask!Task)
{
    cl_event event;
...
unittest
{
    auto queue = makeAQueue();
    Task!someKernel task1;
    Task!someOtherKernel task2;

    // expresses that any instance of task2 must depend on the previous completion of an
    // instace of task1. Would be nicer as a compile-time thing
    task2.dependsOn(task1);
    
    //alternatively, could do something like this:
    /+
    Task!(someKernel, DependsOn!(task1)) task2;
    +/

    //setting these also adds the queue to task1.
    task1.globalRange(queue) = [[0, 100], [3, 7]];
    ...
    //enqueues an instance of task1 on all queues associated with task1
    auto t1Ev = task1(arg1, arg2);

    auto task2i = task2.instance();

    //override the default args
    task2i.setArgs(arg3New);
    
    //Error: haven't set the dependency up
    //task2i.enqueue();

    task2i.dependsOn(t1Ev);

    //checks that dependency is set, everything OK.
    //does a lookup in the tasks queue table to find range info for this queue
    auto t2Ev = queue.enqueue(task2);

    auto someMemory = new Type[n];
    auto results = queue.clRead(resBuffer, someMemory, true, 0, [t2Ev]);
